...
func TestHandlers(t *testing.T) {
	...
	// Test Server :) -> Courtesy of Go
	ts := httptest.NewTLSServer(routes)
	defer ts.Close()

	for _, e := range theTests {
		resp, err := ts.Client().Get(ts.URL + e.url) // appends the test server's port to the target url
		if err != nil {
			t.Log(err)
			t.Fatal(err)
		}
...
func TestRepositoryReservation(t *testing.T) {
	...
	cntx := getCntx(req)
	req = req.WithContext(cntx)

	rr := httptest.NewRecorder()
	session.Put(cntx, "reservation", reservation)
...
func getCntx(req *http.Request) context.Context {
	cntx, err := session.Load(req.Context(), req.Header.Get("X-Session"))
	if err != nil {
		log.Println(err)
	}
...
func TestRepositoryPostReservation(t *testing.T) {
	...
	// tells the server that this is a form post
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler returned wrong response code for missing response body: got %d wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
